...
def fetch_medical():
    print("[dataset_loader] === MEDICAL DATASET (v2) ===")
    records = []

    # 1. MedMCQA — rich MCQ with full explanations
    try:
        print("[dataset_loader] [1/4] Downloading MedMCQA with explanations...")
        ds = load_dataset("medmcqa", split="train[:25000]")
        ...
        for item in ds:
            ...
            opc = item.get("opc", "")
            opd = item.get("opd", "")
            cop = item.get("cop", -1)  # 0-indexed
            exp = item.get("exp", "")
            subject = item.get("subject_name", "")
            topic = item.get("topic_name", "")

            options = [opa, opb, opc, opd]
            if not question or cop < 0 or cop > 3:
                continue
            if not exp or exp.strip() == "":
                continue

            correct_letter = option_labels[cop]
            ...
            added += 1
        print(f"[dataset_loader]   MedMCQA: {added} records")
    except Exception as e:
        print(f"[dataset_loader] Error downloading MedMCQA: {e}")

    # 2. Medical Meadow Flashcards — direct Q&A pairs
    try:
        print("[dataset_loader] [2/4] Downloading Medical Meadow Flashcards...")
        ds2 = load_dataset("medalpaca/medical_meadow_medical_flashcards", split="train[:12000]")
        added = 0
        for item in ds2:
            inp = item.get("input", "")
            out = item.get("output", "")
            if inp and out and len(out) >= 30:
                records.append({
                    "id": f"med_{uuid.uuid4().hex[:8]}",
                    "text": inp,
                    "label": out,
                ...
                added += 1
        print(f"[dataset_loader]   Flashcards: {added} records")
    except Exception as e:
        print(f"[dataset_loader] Error downloading Medical Flashcards: {e}")

    # 3. ChatDoctor — Real doctor-patient conversations with clinical reasoning
    try:
        print("[dataset_loader] [3/4] Downloading ChatDoctor-HealthCareMagic...")
        ds3 = load_dataset("lavita/ChatDoctor-HealthCareMagic-100k", split="train[:10000]")
        added = 0
        for item in ds3:
            # Try common field name patterns
            inp = item.get("input", "") or item.get("instruction", "") or item.get("question", "")
            out = item.get("output", "") or item.get("answer", "") or item.get("response", "")
            if inp and out and len(out) >= 50:
                records.append({
                    "id": f"med_{uuid.uuid4().hex[:8]}",
                    "text": f"Patient presents with: {inp}" if not inp.startswith("Patient") else inp,
                    "label": out,
                    "domain": "medical"
                })
                added += 1
        print(f"[dataset_loader]   ChatDoctor: {added} records")
    except Exception as e:
        print(f"[dataset_loader] Error downloading ChatDoctor: {e}")

    # 4. Medical Meadow WikiDoc — Encyclopedia-style medical knowledge
    try:
        print("[dataset_loader] [4/4] Downloading Medical Meadow WikiDoc...")
        ds4 = load_dataset("medalpaca/medical_meadow_wikidoc", split="train[:5000]")
        added = 0
        for item in ds4:
            inp = item.get("input", "") or item.get("instruction", "")
            out = item.get("output", "") or item.get("answer", "")
            if inp and out and len(out) >= 50:
                records.append({
                    "id": f"med_{uuid.uuid4().hex[:8]}",
                    "text": inp,
                ...
                added += 1
        print(f"[dataset_loader]   WikiDoc: {added} records")
    except Exception as e:
        print(f"[dataset_loader] Error downloading WikiDoc: {e}")

    # 5. MedQA USMLE (Differential Diagnosis edge cases)
    try:
        print("[dataset_loader] [5/5] Downloading MedQA USMLE...")
        ds5 = load_dataset("GBaker/MedQA-USMLE-4-options", split="train[:5000]")
        added = 0
        for item in ds5:
            question = item.get("question", "")
            ans = item.get("answer", "")
            ...
            if question and ans:
                ...
                records.append({
                    "id": f"med_{uuid.uuid4().hex[:8]}",
                    "text": f"Evaluate the following clinical scenario and provide the diagnosis:\\n{question}\\nOptions:\\n{opt_str}",
                    "label": f"The correct diagnosis/step is: {ans}.",
                    "domain": "medical"
                })
                added += 1
        print(f"[dataset_loader]   MedQA USMLE: {added} records")
    except Exception as e:
        print(f"[dataset_loader] Error downloading MedQA USMLE: {e}")

    records = _quality_filter(records)
    records = _convert_to_cot(records, fraction=0.30)
    _jsonl_write(records, "data/processed/medical.jsonl")
